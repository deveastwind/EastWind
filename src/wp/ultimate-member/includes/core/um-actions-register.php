...
function postIndividualArtemis ( $user_id, $args){

  $date_of_birth = new DateTime($args['birth_date']);
  //PC::debug( date_format($date_of_birth, 'd/m/y'));
  //PC::debug($args['gender'][0]);
  // Step 1: Assess Indiv API
  $fields = array(
    "domain_name"=> "HASTINGS_UAT",
    "rfrID"=> $user_id, 
    "first_name"=> $args['first_name'],
    "last_name"=> $args['last_name'], 
    "country_of_residence"=> $args['country'], 
    "date_of_birth"=> date_format($date_of_birth, 'd/m/Y'),
    "gender"=> strtoupper($args['gender'][0]),
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => "https://p4.cynopsis.co/artemis_hastings_uat/default/individual_risk",
    ...
    CURLOPT_HTTPHEADER => array(
      ...
      "postman-token: 08d6148b-10c2-2bbe-385b-44adb945016a",
      "web2py-user-token: b10a858d-5289-46fb-a9e7-cfb95e846457"
    ),
  ));
...
function postIndividualDocSelfie ($user_id, $args){

  //Step 2a: Indiv DOC POST; Post SELFIE
  $fields = array(
    "cust_rfr_id"=> $user_id,
    "document_type"=> "SELFIE"
  );

  $selfie_name = basename($args['selfie']);
  $selfie_path = "https://dev.eastwind.host/wp-content/uploads/ultimatemember/" .$user_id. "/" .$selfie_name;
  
  $files = array('file' => file_get_contents($selfie_path));
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => "https://p4.cynopsis.co/artemis_hastings_uat/api/individual_doc",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
    ...
    CURLOPT_HTTPHEADER => array(
      ...
      "postman-token: b5e2df98-f315-6f22-bb8f-ccd7525b4f4c",
      "web2py-user-token: b10a858d-5289-46fb-a9e7-cfb95e846457"
    ),
  ));
...
function postIndividualDocID ($user_id, $args){

  //Step 2a: Indiv DOC POST; Post photo_id
  $fields = array(
    "cust_rfr_id"=> $user_id,
    "document_type"=> "PASSPORT"
  );

  $photoid_name = basename($args['photo_id']);
  $photoid_path = "https://dev.eastwind.host/wp-content/uploads/ultimatemember/" .$user_id. "/" .$photoid_name;
  
  $files = array('file' => file_get_contents($photoid_path));
...
function postIndividualFace ($user_id, $source_id, $target_id){
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => "https://p4.cynopsis.co/artemis_hastings_uat/api/individual_face",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
    ...
    CURLOPT_HTTPHEADER => array(
      "cache-control: no-cache",
      "content-type: application/json",
      "postman-token: 59560ad7-4bf1-0256-68c9-e871401fd9bb",
      "web2py-user-token: b10a858d-5289-46fb-a9e7-cfb95e846457"
    ),
  ));
...
function postIndividualReport($user_id){
  ...
  curl_setopt_array($curl, array(
    CURLOPT_URL => "https://p4.cynopsis.co/artemis_hastings_uat/api/individual_customer_report",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => "",
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
    ...
    CURLOPT_HTTPHEADER => array(
      "cache-control: no-cache",
      "content-type: application/json",
      "postman-token: 4edc9bf2-0a5c-f73b-0f33-3895ddedd292",
      "web2py-user-token: b10a858d-5289-46fb-a9e7-cfb95e846457"
    ),
  ));
...
function kycNewRegistration ($user_id, $args){
  PC::debug($args);

  if(postIndividualArtemis($user_id, $args)){
    PC::debug("Individual Data Successful!");
    $target_Selfie = postIndividualDocSelfie($user_id,$args);
    if ($target_Selfie){
      PC::debug("Selfie Successful!");
      $source_Doc = postIndividualDocID($user_id,$args);
      if($source_Doc){
        PC::debug("Document Successful!");
        if(postIndividualFace($user_id, $source_Doc, $target_Selfie)){
          postIndividualReport($user_id);
        }
      }
    }
  }
}
add_action('um_registration_complete','kycNewRegistration', 10, 2);
